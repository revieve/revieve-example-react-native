...
<html lang="en">
  ...
  <body>
    ...
    <script type="text/javascript">
      var config = {
        partner_id: '5HfJcydgtT',
        experienceId: 'experience_demo',
        env: 'test',
        onTrackingEvent: function(event) { console.log(event.payload)}
      };
            ...
            rv.onload = rv.onreadystatechange = function() {
              try {    
                  ...
                  Revieve.Init(config, function() {
                    window.ReactNativeWebView.postMessage(JSON.stringify( {'type': 'Loaded', 'data': "" }));
                    Revieve.API.show();
                  });
...
export function MakeUp() {
  ...
  const [productId, setProductId] = useState("0273140006");

  useEffect(() => {
    if (!webviewRef.current) return;
    if (!loaded) return;
    webviewRef.current.injectJavaScript(
      `window.Revieve.API.liveAR.addTryOnProduct("${productId}")`
    );
  }, [productId, loaded]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.controls}>
        <TouchableOpacity
          style={[styles.button, productId === "0273140006" && styles.selected]}
          onPress={() => setProductId("0273140006")}
        >
          <Text style={productId === "0273140006" && styles.textSelected}>
            TryOn Product 1
          </Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={[styles.button, productId === "0273140009" && styles.selected]}
          onPress={() => setProductId("0273140009")}
        >
          <Text style={productId === "0273140009" && styles.textSelected}>
            TryOn Product 2
          </Text>
        </TouchableOpacity>
      </View>
